  ...
  useEffect(() => {
    ...
    const color_frag = `
      ...
      void main() {
        ...
        // ── Iridescence: thin-film rainbow on fluid flow edges ───────────────
        // hue = direction the fluid is flowing → different ripple edges = different colors
        float hue     = atan(vel.y, vel.x) / 6.2832 + 0.5;
        vec3  iriCol  = rainbow(hue);
        // mask: only appears where fluid is actively moving
        float iriMask = clamp(lenv * 3.5, 0.0, 1.0) * lenv;
        outRGB += iriCol * iriMask * 0.35;
        outA = clamp(outA + iriMask * 0.25, 0.0, 1.0);
    ...
    class Advection extends ShaderPass {
      ...
      constructor(simProps: Record<string, unknown>) {
        super({
          material: { vertexShader: face_vert, fragmentShader: advection_frag, uniforms: { boundarySpace: { value: simProps.cellScale }, px: { value: simProps.cellScale }, fboSize: { value: simProps.fboSize }, velocity: { value: (simProps.src as THREE.WebGLRenderTarget).texture }, dt: { value: simProps.dt }, isBFECC: { value: true } } },
          output: simProps.dst
        });
        this.uniforms = (this.props.material as Record<string, unknown>).uniforms as Uniforms;
        this.init();
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(simProps?: Record<string, unknown>) {
        ...
        const mouseM = new THREE.RawShaderMaterial({ vertexShader: mouse_vert, fragmentShader: externalForce_frag, blending: THREE.AdditiveBlending, depthWrite: false, uniforms: { px: { value: simProps.cellScale }, force: { value: new THREE.Vector2(0, 0) }, center: { value: new THREE.Vector2(0, 0) }, scale: { value: new THREE.Vector2(simProps.cursor_size as number, simProps.cursor_size as number) } } });
    ...
    class Viscous extends ShaderPass {
      constructor(simProps: Record<string, unknown>) {
        super({ material: { vertexShader: face_vert, fragmentShader: viscous_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, velocity: { value: (simProps.src as THREE.WebGLRenderTarget).texture }, velocity_new: { value: (simProps.dst_ as THREE.WebGLRenderTarget).texture }, v: { value: simProps.viscous }, px: { value: simProps.cellScale }, dt: { value: simProps.dt } } }, output: simProps.dst, output0: simProps.dst_, output1: simProps.dst });
        this.init();
      }
    ...
    class Divergence extends ShaderPass {
      constructor(simProps: Record<string, unknown>) {
        super({ material: { vertexShader: face_vert, fragmentShader: divergence_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, velocity: { value: (simProps.src as THREE.WebGLRenderTarget).texture }, px: { value: simProps.cellScale }, dt: { value: simProps.dt } } }, output: simProps.dst });
        this.init();
      }
    ...
    class Poisson extends ShaderPass {
      constructor(simProps: Record<string, unknown>) {
        super({ material: { vertexShader: face_vert, fragmentShader: poisson_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, pressure: { value: (simProps.dst_ as THREE.WebGLRenderTarget).texture }, divergence: { value: (simProps.src as THREE.WebGLRenderTarget).texture }, px: { value: simProps.cellScale } } }, output: simProps.dst, output0: simProps.dst_, output1: simProps.dst });
        this.init();
      }
    ...
    class Pressure extends ShaderPass {
      constructor(simProps: Record<string, unknown>) {
        super({ material: { vertexShader: face_vert, fragmentShader: pressure_frag, uniforms: { boundarySpace: { value: simProps.boundarySpace }, pressure: { value: (simProps.src_p as THREE.WebGLRenderTarget).texture }, velocity: { value: (simProps.src_v as THREE.WebGLRenderTarget).texture }, px: { value: simProps.cellScale }, dt: { value: simProps.dt } } }, output: simProps.dst });
        this.init();
      }
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: Record<string, unknown>) {
        ...
        this.autoDriver = new AutoDriver(Mouse, this as unknown as WebGLManager, { enabled: props.autoDemo as boolean, speed: props.autoSpeed as number, resumeDelay: props.autoResumeDelay as number, rampDuration: props.autoRampDuration as number });
        this.init();
    ...
    webglRef.current = webgl as unknown as LiquidEtherWebGL;
    ...
    const io = new IntersectionObserver(entries => {
      ...
      if (visible && !document.hidden) (webglRef.current as unknown as WebGLManager).start();
      else (webglRef.current as unknown as WebGLManager).pause();
    ...
    const ro = new ResizeObserver(() => {
      ...
      resizeRafRef.current = requestAnimationFrame(() => { if (webglRef.current) webgl.resize(); });
    });
